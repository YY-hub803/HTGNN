import torch


def split_dataset(X, Y, train_ratio=0.6, val_ratio=0.2):
    """
    按比例切分数据集为 Train (60%), Val (20%), Test (20%)
    返回切割后的数据以及对应的 Mask（掩码在 PyG 中很常用）
    """
    num_samples = X.shape[0]
    train_end = int(num_samples * train_ratio)
    val_end = train_end + int(num_samples * val_ratio)

    X_scaled = X.clone()
    X_train = X_scaled[:train_end]
    Y_train = Y[:train_end]
    x_mean = X_train.mean(dim=(0,1), keepdim=True)
    x_std = X_train.std(dim=(0,1), keepdim=True)
    y_mean = Y_train.mean(dim=(0,1), keepdim=True)
    y_std = Y_train.std(dim=(0,1), keepdim=True)

    x_std[x_std == 0] = 1e-8
    y_std[y_std == 0] = 1e-8

    X_scaled = (X_scaled - x_mean) / x_std
    Y_scaled = (Y_train - y_mean) / y_std

    data_splits = {
        'train_x': X_scaled[:train_end],
        'train_y': Y_scaled[:train_end],
        'val_x': X_scaled[train_end:val_end],
        'val_y': Y_scaled[train_end:val_end],
        'test_x': X_scaled[val_end:],
        'test_y': Y_scaled[val_end:]
    }
    train_stats = {
        "x_mean": x_mean.tolist(),
        "x_std": x_std.tolist(),
        "y_mean": y_mean.tolist(),
        "y_std": y_std.tolist(),
    }

    train_mask = torch.zeros(num_samples, dtype=torch.bool)
    val_mask = torch.zeros(num_samples, dtype=torch.bool)
    test_mask = torch.zeros(num_samples, dtype=torch.bool)

    train_mask[:train_end] = True
    val_mask[train_end:val_end] = True
    test_mask[val_end:] = True

    masks = {
        'train': train_mask,
        'val': val_mask,
        'test': test_mask
    }

    print(f"切分完成: 训练集 {train_end} 条, 验证集 {val_end - train_end} 条, 测试集 {num_samples - val_end} 条")
    return data_splits, masks, train_stats


def create_sliding_windows(X, Y, window_size,pred_len):
    xs, ys = [], []
    T = X.size(1)
    for t in range(T - window_size-pred_len+1):
        xs.append(X[:,t:t+window_size,:])
        ys.append(Y[:,t+window_size : t+window_size+pred_len,:])

    X_seq = torch.stack(xs)
    Y_seq = torch.stack(ys)
    print(f"滑动窗口生成完毕！")
    print(f"输入 X 形状: {X_seq.shape} -> (Samples,Num_sites,Sequence_length,Num_Features)")
    print(f"标签 Y 形状: {Y_seq.shape} -> (Samples,Num_sites,Pred_length,Num_Targets)")
    return X_seq, Y_seq

def get_windows(X,Y,train_ratio, val_ratio,window_size, pred_len):
    data_splits, masks, train_stats = split_dataset(X, Y, train_ratio, val_ratio)
    Train_sample_x,Train_sample_y = create_sliding_windows(data_splits['train_x'],data_splits['train_y'], window_size, pred_len)
    Val_sample_x,Val_sample_y = create_sliding_windows(data_splits['val_x'], data_splits['val_y'], window_size, pred_len)
    Test_sample_x,Test_sample_y = create_sliding_windows(data_splits['test_x'], data_splits['test_y'], window_size, pred_len)

    Sample_data = {
        'train_x': Train_sample_x,
        'train_y': Train_sample_y,
        'val_x': Val_sample_x,
        'val_y': Val_sample_y,
        'test_x': Test_sample_x,
        'test_y': Test_sample_y
    }

    return Sample_data,data_splits, masks, train_stats
