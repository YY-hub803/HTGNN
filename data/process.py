...
def split_dataset(X, Y, X_city,X_city_static,train_ratio=0.6, val_ratio=0.2):
    ...
    X_static_train = X_static_scaled[:train_end]
    ...
    x_static_mean = X_static_train.mean(dim=(0,1), keepdim=True)
    x_static_std = X_static_train.std(dim=(0,1), keepdim=True)
    ...
    data_splits = {
        ...
        'train_x_static': X_static_scaled[:train_end],
        'val_x_static': X_static_scaled[train_end:val_end],
        'test_x_static': X_static_scaled[val_end:],
    }
    train_stats = {
...
def create_sliding_windows(X, Y,x_city, x_static,window_size,pred_len):
    xs, ys,xs_city,xs_static = [], [],[],[]
    T = X.size(1)
    ...
    return X_seq, Y_seq,X_city_seq,X_static_seq
...
def get_windows(X,Y,X_city,X_city_static,train_ratio, val_ratio,window_size, pred_len):
    ...
    Train_sample_x,Train_sample_y,Train_sample_x_city,Train_sample_x_static = create_sliding_windows(data_splits['train_x'],data_splits['train_y'],data_splits['train_x_city'],data_splits['train_x_static'] ,window_size, pred_len)
    Val_sample_x,Val_sample_y ,Val_sample_x_city,Val_sample_x_static= create_sliding_windows(data_splits['val_x'], data_splits['val_y'],data_splits['val_x_city'],data_splits['val_x_static'], window_size, pred_len)
    Test_sample_x,Test_sample_y,Test_sample_x_city,Test_sample_x_static = create_sliding_windows(data_splits['test_x'], data_splits['test_y'], data_splits['test_x_city'],data_splits['test_x_static'],window_size, pred_len)
    ...
    Sample_data = {
        ...
        'train_X_static':Train_sample_x_static,
        'val_X_static':Val_sample_x_static,
        'test_X_static':Test_sample_x_static,
    }
